...
import {
    QifImportService,
    ImportedTransactionsFileDto,
    StagedImportReviewDto,
    StagingRowDto,
    UpdateStagingRowRequest
} from '../../../services/qif-import.service';
...
@Component({
    ...
    templateUrl: './qif-import.component.html',
...
export class QifImportComponent implements OnInit {
    ...
    // ── Step 2 ──────────────────────────────────────────────────────────────────
    importFile: ImportedTransactionsFileDto | null = null;
    reviewData: StagedImportReviewDto | null = null;
    loadingReview = false;

    validDataSource = new MatTableDataSource<StagingRowDto>();
    duplicateDataSource = new MatTableDataSource<StagingRowDto>();
    alreadyImportedDataSource = new MatTableDataSource<StagingRowDto>();
    errorDataSource = new MatTableDataSource<StagingRowDto>();
    ...
    displayedValidColumns = ['date', 'description', 'amountRaw', 'counterAccount', 'keyword', 'category', 'actions'];
    ...
    onCancelAttempt(): void {
        if (!this.importFile) {
            // Nothing uploaded yet — safe to close immediately
            this.dialogRef.close(false);
            return;
        }
        // Import in progress — ask for confirmation
        const confirmRef = this.dialog.open(CancelConfirmDialogComponent, {
            disableClose: true,
    ...
    onFileSelected(event: Event): void {
        ...
        if (file) {
            if (!file.name.toLowerCase().endsWith('.qif')) {
                this.snackBar.open('Only .qif files are supported', 'Close', { duration: 4000 });
                input.value = '';
                return;
            }
    ...
    upload(stepper: any): void {
        ...
        this.uploading = true;
        const { accountId, allowDuplicates, dateFormat } = this.uploadForm.getRawValue();

        this.qifService.upload(this.selectedFile, accountId, allowDuplicates, dateFormat).subscribe({
            next: (file) => {
    ...
    private loadReview(fileId: string, stepper?: any): void {
        ...
        this.qifService.getStagedImport(fileId).subscribe({
            next: (data) => {
    ...
    applyFilter(source: MatTableDataSource<StagingRowDto>, term: string): void {
        source.filterPredicate = (row, f) => row.description.toLowerCase().includes(f);
        source.filter = term.trim().toLowerCase();
    }

    updateRowCounterAccount(row: StagingRowDto, accountId: string): void {
        ...
        this.qifService.updateStagingRow(this.importFile.id, row.id, request).subscribe({
            next: (updated) => {
    ...
    addKeywordForRow(row: StagingRowDto, keyword: string): void {
        ...
        this.keywordRuleService.create(request).subscribe({
            next: () => {
                ...
                // Apply keyword to all unassigned rows in the current review that match
    ...
    forceImport(row: StagingRowDto): void {
        if (!this.importFile) return;
        const request: UpdateStagingRowRequest = {
    ...
    isInflow(row: StagingRowDto): boolean {
    ...
    commit(): void {
        ...
        this.qifService.commitImport(this.importFile.id).subscribe({
            next: () => {
                ...
                this.dialogRef.close(true); // signal caller to refresh
            },
            error: (err) => {
                this.committing = false;
